...
def _import_module_and_get_func(job_name):
    ...
    if str(filename) + ".py" in os.listdir(tasksdir):
        ...
        try:
            ...
            log(
                f"Error importing '{module_name}' from scheduler tasks directory: Exception:{type(my_ex)}:{my_ex}",
                "critical",
            )
    else:
        modulelist = job_name.split(".")
        ...
        try:
            ...
            log(
                f"Error importing '{job_name}' from scheduler tasks directory: Exception:{type(my_ex)}:{my_ex}",
                "critical",
            )
